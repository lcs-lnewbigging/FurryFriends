...
struct DogView: View {
    ...
    @State var currentFurryFriends: FurryFriends  = FurryFriends(message: "",
                                                                 status: "")
    
    // This will keep track of our list of favourite jokes
    @State var favourites: [FurryFriends] = []   // empty list to start
    
    // This will let us know whether the current exists as a favourite
    @State var currentFurryFriendsAddedToFavourites: Bool = false
    ...
    // Define the function "loadFurryFriends"
    ...
    func loadNewFurryFriends() async {
        ...
        do {
            ...
            //                                         |
            //                                         V
            currentFurryFriends = try JSONDecoder().decode(FurryFriends.self, from: data)
            
            
            
            // Reset the flag that tracks whether the current joke
            // is a favourite
            currentFurryFriendsAddedToFavourites = false
    ...
    func loadFavourites() {
        ...
        do{
            ...
            favourites = try JSONDecoder().decode([FurryFriends].self, from: data)
    ...
    var body: some View {
        ScrollView{
            VStack {
                ...
                Text(currentFurryFriends.message)
                    .font(.title)
                ...
                //                      CONDITION                        true   false
                    .foregroundColor(currentFurryFriendsAddedToFavourites == true ? .red : .secondary)
                    .onTapGesture {
                        
                        // Only add to the list if it is not already there
                        if currentFurryFriendsAddedToFavourites == false {
                            
                            // Adds the current joke to the list
                            favourites.append(currentFurryFriends)
                            
                            // Record that we have marked this as a favourite
                            currentFurryFriendsAddedToFavourites = true
